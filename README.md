...
当前发布版本：**[0.1.1 · PyPI](https://pypi.org/project/wx-downloder/0.1.1/)**。已验证可以直接从 PyPI 安装，无需克隆源码。

[GitHub 仓库](https://github.com/QinGeneral/wx-downloder) · [发布工作流](https://github.com/QinGeneral/wx-downloder/actions/workflows/release.yml)
...
```bash
uv tool install wx-downloder==0.1.1
```

也可以在 Python 虚拟环境中使用 `python -m pip install wx-downloder`。

未安装 Chrome 时，可以安装 Chromium 并指定登录浏览器：

```bash
uv tool run --from wx-downloder playwright install chromium
wx-downloder config set browser chromium
```

如果终端提示找不到 `wx-downloder`，运行 `uv tool update-shell` 后重新打开终端。
...
默认成品目录为 `~/Downloads/wx-channels`。首次下载时若没有可用的本地凭据，会自动打开登录窗口。服务端登录状态过期后，重新运行 `wx-downloder login`。
...
```bash
wx-downloder download --input links.json
...
`wx-downloder download` 不带参数时，在终端提示输入链接或 JSON；通过管道调用时读取标准输入。程序先验证全部输入，再开始登录和下载。

## 目录与下载配置

```bash
# 配置成品目录和临时缓存目录
wx-downloder config set download_dir '~/Downloads/微信视频'
wx-downloder config set cache_dir '~/Downloads/视频缓存'

# 仅本次任务覆盖目录和视频编码偏好
wx-downloder download --input links.json --output './videos' --cache-dir './cache' --quality h264
...
配置方式为 `wx-downloder config set 配置项 值`，例如：

```bash
wx-downloder config set timeout 120
wx-downloder config set retries 3
...
wx-downloder login --browser chrome --timeout 300
...
```bash
uv tool upgrade wx-downloder
```

如果之前通过本地源码安装，或需要取消固定版本并切换到 PyPI 安装：

```bash
uv tool install --force wx-downloder --default-index https://pypi.org/simple
wx-downloder --version
```

旧命令 `wx-channels` 已改名为 `wx-downloder`。安装并验证新版后，使用 `uv tool uninstall wx-channels-cli` 移除旧工具。配置目录和 `WX_CHANNELS_HOME` 保持兼容，原登录凭据仍有效时无需重新扫码。
...
`0.1.1` 发布前通过了 39 项常规测试和 1 项真实 Chrome 登录关闭回归测试，wheel 和源码包均完成独立安装验证。真实视频实测下载 8,155,895 字节，ffprobe 确认含 H.264 视频和 AAC 音频，时长 73.45 秒。发布后再次从 PyPI 隔离安装，`wx-downloder --version` 输出 `0.1.1`。

## 发布流程

[GitHub Actions 发布工作流](https://github.com/QinGeneral/wx-downloder/actions/workflows/release.yml) 位于 `.github/workflows/release.yml`，已配置 PyPI Trusted Publishing：`QinGeneral / wx-downloder / release.yml / pypi`。
...
[0.1.1 发布记录](https://github.com/QinGeneral/wx-downloder/actions/runs/35481356579) · [PyPI 发行文件](https://pypi.org/project/wx-downloder/0.1.1/#files)
